...
set
  ...
  username_search = lower(coalesce(username, regexp_replace(split_part(email, '@', 1), '[^a-zA-Z0-9_]', '', 'g'))),
  display_name = coalesce(username, display_name)
where username is null;

update public.profiles
set username_search = lower(username)
where username is not null and username_search is null;
